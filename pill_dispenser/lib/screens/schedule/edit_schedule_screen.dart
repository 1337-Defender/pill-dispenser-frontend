...
class _EditScheduleScreenState extends State<EditScheduleScreen> {
  ...
  void initState() {
    super.initState();
    _fetchMedCompOptionsAndSchedule();
  }

  Future<void> _fetchMedCompOptionsAndSchedule() async {
    final userId = Provider.of<AuthProvider>(context, listen: false).currentUser?.id;
    final dispenserId = Provider.of<AuthProvider>(context, listen: false).dispenserId;
    if (userId == null) return;
    final medCompResult = await Supabase.instance.client
        .from('compartments')
    ...
    final medCompOptions = List<Map<String, dynamic>>.from(medCompResult);
    final scheduleResult = await Supabase.instance.client
        .from('schedules')
        .select()
        .eq('id', widget.scheduleId)
        .single();
    // Find the med+comp option matching the schedule
    final medComp = medCompOptions.firstWhere(
      (item) => item['id'] == scheduleResult['compartment_id'] && item['medication_id'] == scheduleResult['medication_id'],
      orElse: () => <String, dynamic>{},
    );
    setState(() {
      _medCompOptions = medCompOptions;
      _selectedMedComp = medComp;
      _selectedDays = (scheduleResult['days_of_week'] as List<dynamic>?)?.cast<int>() ?? [];
      final dispenseTime = scheduleResult['dispense_time'] as String?;
      ...
      _quantity = scheduleResult['quantity_to_dispense'] ?? 1;
      _startDate = DateTime.tryParse(scheduleResult['start_date'] ?? '') ?? DateTime.now();
      if (scheduleResult['end_date'] != null) {
        _endDate = DateTime.tryParse(scheduleResult['end_date']);
  ...
  Future<void> _save() async {
    ...
    await Supabase.instance.client.from('schedules').update({
      ...
      'end_date': _repeatForever ? null : _endDate?.toIso8601String().substring(0, 10),
    }).eq('id', widget.scheduleId);
